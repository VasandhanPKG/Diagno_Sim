  ...
  async generateCase(diseaseName: string) {
    ...
    return withRetry(async () => {
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction: "You are a medical case generator. Generate concise, realistic medical cases in JSON format. Ensure the history is detailed but not excessively long (max 500 words). This history will be hidden from the student and used by the patient AI to answer questions.",
          ...
          responseSchema: {
            ...
            required: ["patientName", "age", "gender", "severity", "vitals", "chiefComplaint", "history"]
          }
        }
      });
  ...
  async getPatientResponse(caseInfo: any, chatHistory: any[], studentMessage: string) {
    ...
    return withRetry(async () => {
      const response = await ai.models.generateContent({
        ...
        [STRICT MEDICAL HISTORY]
        ${caseInfo.history}
        
        [INSTRUCTIONS]
        1. Respond as the patient. Be realistic, sometimes vague, sometimes anxious.
        2. Check the [STRICT MEDICAL HISTORY] first for any facts (symptoms, duration, past illnesses, family history).
        3. If the student asks something covered in the history, answer accordingly.
        4. If the student asks something NOT in the history, use your general medical knowledge to provide a consistent, realistic answer for someone with ${caseInfo.chiefComplaint} and ${caseInfo.severity} severity.
        5. Don't use medical terminology yourself unless the patient would know it.
